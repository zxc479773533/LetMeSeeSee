...
int main(int argc, char **argv) {
  ...
  auto req = SendHTTPRequest(conn, HTTPRequest{}.AutoFill().Header("CMD","nodelist").Page("/"));
  ...
  auto req1 = SendHTTPRequest(conn, HTTPRequest{}.AutoFill().Page("/Save_test1").Header("CMD","Save_test1"));
  get_data(req1.Serialize().std_string());
  auto req2 = SendHTTPRequest(conn, HTTPRequest{}.AutoFill().Page("/Save_test2").Header("CMD","Save_test2"));
  get_data(req2.Serialize().std_string());
}